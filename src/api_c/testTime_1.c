...
double get_query_time_unified_table() {
    MYSQL* CONN = conn();
    clock_t start = clock();
    if (mysql_query(CONN, "SELECT * FROM tabela_unificada WHERE status_producao = 'Produzido'") != 0) {
...
double get_query_time() {
    MYSQL* CONN = conn();
    clock_t start = clock();
    if (mysql_query(CONN, "select * from produzidos") != 0) {
        fprintf(stderr, "Erro ao executar a consulta: %s\n", mysql_error(CONN));
        mysql_close(CONN);
        exit(1);
    }
